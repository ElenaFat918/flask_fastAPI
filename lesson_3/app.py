...
from lesson_3.models import db, User, Post, Comment

app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///../../instance/first_db.db'
db.init_app(app)  # инициализация , связь с приложением при импорте таблиц в приложении
...
@app.cli.command("init-db")  # модуль, позволяет использовать команду Flask для запуска в команд строке
def init_db():
    db.create_all()
    print('OK')
...
def edit_user():
    user = User.query.ﬁlter_by(username='john').ﬁrst()
